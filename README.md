...
> [!TIP]
> **小技巧：更新规则集等远程资源请使用 [使用配置](#使用配置) 功能。若当前配置是远程配置，希望使用 [自动更新](#自动更新) 功能定期更新规则集、脚本集等远程资源，且不希望自定义设置被覆盖或丢失时，可进入 [纯文本编辑](#编辑纯文本) 删除或注释掉 `update-url = *`，然后再使用 [自动更新](#自动更新) 功能。或者也可以考虑使用 [扩展配置/包含配置](#扩展配置)**
