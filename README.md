...
> 点击 **使用配置** 可以启用该配置文件，也可以使用该按钮对当前使用的远程 [规则集](#规则集url)、[脚本](#脚本url) 等远程资源进行更新
...
> [!TIP]
> * **若仅需要更新规则集等远程资源，请使用 [使用配置](#使用配置) 功能**
...
> 根据不同 App 或特定服务指定分流规则，并使其按照设定的规则执行代理策略。详解请参考条目：[规则与代理分组](#规则与代理分组)
> 
...
> 
> **random**：随机使用分组里的不同节点进行连接，相同的域名可能使用不同的节点。此类型不推荐包含其他代理分组

### [负载均衡](#使用目录)

> [代理分组类型](#代理分组类型) 中的 `load-balance` 表示不同规则的请求使用分组里的不同节点进行连接，能够尽可能将流量平均分配给所有子策略或代理。相同的域名会使用同一个节点
> 
> 使用负载均衡的方法：
> 
...
> 
> 当前使用的所有远程规则资源的展示页面，其后显示的数字为当前生效的数量和总共包含的数量。进入后所显示的 ✅ 表示当前规则状态生效，❎ 表示当前规则状态失效。点击相应地址可重新拉取并弹出状态提示，但不会重新编译更新配置文件，点击 [使用配置](#使用配置)，可以对当前所有 URL 进行更新并重新编译更新配置文件

### [脚本URL](#使用目录)

> 配置 > 配置文件 > 编辑配置 > 脚本 URL
> 
> 当前使用的所有远程脚本资源的展示页面，其后显示的数字为当前生效的数量和总共包含的数量。进入后所显示的 ✅ 表示当前脚本状态生效，❎ 表示当前脚本状态失效。点击相应地址可重新拉取并弹出状态提示，但不会重新编译更新配置文件，点击 [使用配置](#使用配置)，可以对当前所有 URL 进行更新并重新编译更新配置文件
...
> 模块是指为 Shadowrocket 提供额外功能的插件或者扩展项，一般是以增强或自定义软件的使用体验为出发点。模块的写法与 [配置文件](#配置文件) 相同，且模块的 [规则优先级](#规则优先级) 高于配置文件。一般情况下，模块仅包含完整配置文件格式中的特定部分，以实现特定的特殊效果且不影响配置文件的其他功能。用户可以在模块里另外设置 [通用参数](#通用参数) 以及添加 [规则](#添加规则)、[Hosts](#hosts)、[URL重写](#url重写)、[证书内容](#证书模块)、解密主机名、脚本等内容
>
> **下载远程模块**
> > * 配置 > 模块 > 右上角 `➕` > 填写链接 > 下载
> 
...
>     * 更新间隔：单位为  `天`，可选 1-7 天
>     * 若当前配置文件是不含更新地址的本地配置文件，则自动更新功能仅会更新当前所用的规则集、脚本等远程资源，参见：[使用配置](#使用配置)、[规则集URL](#规则集url)、[脚本URL](#脚本url)
>     * 规则集/脚本资源手动更新方法：点击 [配置文件](#配置文件) > [使用配置](#使用配置)
...
> 以下内容默认指的是 [全局路由](#全局路由) 设置为配置模式的情况下
> 
> **流量处理流程**
> > * **请求发出**：用户的网络请求需要被处理
> > * **规则匹配**：Shadowrocket 从上到下匹配配置的规则，找到第一个符合条件的规则
> > * **确定策略**：根据匹配规则设定的 [规则策略](#规则策略)，确定使用何种策略进行连接
> > * **策略解析**：
> >   * *如果策略是 **DIRECT**，直接连接，不使用代理*
> >   * *如果策略是 **REJECT**，阻止连接*
> >   * *如果策略是 **代理节点**，使用该代理服务器进行连接*
> >   * *如果策略是 **代理分组**，按照代理分组的类型和设置，选择具体的代理服务器*
> > * **转发请求**：根据确定的代理服务器，处理并转发网络请求
> 
> **策略制约规则**
> > * **规则** 是匹配网络请求的必要条件。当发出网络请求时，Shadowrocket 会根据 **[规则优先级](#规则优先级)** 逐一匹配
> > * **[规则策略](#规则策略)** 可以是固定的 **代理节点**，也可以是一个 **[代理分组](#代理分组)** 或者 **PROXY 默认节点**、**DIRECT 直连** 和 **REJECT 拒绝** 等
> > * 当规则匹配后，如果策略是一个代理节点，流量将通过指定的代理节点进行连接。如果策略是一个代理分组，流量将根据代理分组的 [类型](#代理分组类型) 和设置，选择合适的代理服务器。如果策略是 **拒绝** 类型，则流量将被拒绝连接
> 
> **代理分组的子策略**
> > * **代理分组** 中可以包含 **代理节点** 或 **其他代理分组**，形成包含子策略的层级结构
> >   * *例如：[代理分组类型](#代理分组类型) 为 **Select 手动选择**，它包含了 **节点-01** 和 **节点-02** 两个节点，用户可以手动选择使用其中的哪个代理节点来设定该分组的策略*
> > * **代理分组** 中可以设置经过多层代理分组的嵌套，形成多层级结构，由末端代理分组最终确定具体的代理服务器节点
> >   * *例如：[代理分组类型](#代理分组类型) 为 **Select 手动选择**，它包含了 **美国地区** 和 **日本地区** 两个分组，用户可以手动选择使用其中的哪个代理分组来设定该分组的策略*
> > * **需要注意**，嵌套的代理分组的子策略中，仅推荐在 **末端代理分组** 中使用 **除 Select 之外的其他 [代理分组类型](#代理分组类型)**，以免产生不必要的自动切换
> >   * *例如：设置 **谷歌服务** 代理分组，类型为 **Select**，它包含了 **美国地区** 和 **日本地区** 两个分组，用户指定了其中的 **美国地区**，而 **美国地区** 使用了 **Url-Test** 类型自动测试并使用其包含的 **US01、US02、US03** 三个节点。此时，若前方 **谷歌服务** 的 **Select** 使用了其他代理分组类型，则 **谷歌服务** 可能会在 **美国和日本** 两个代理分组所包含的代理节点内根据设置自动切换，导致 **谷歌服务** 在美国和日本节点间互跳，这可能造成错误的服务分流*
...
> * 若 [模块](#模块) 确认有效且解密设置无误时存在模块失效的情况，则可能是加载资源时因内存、网络等原因导致部分内容出现漏编译的情况。尝试对当前配置点击 [使用配置](#使用配置)，以使远程资源进行重新拉取，一般可恢复正常
