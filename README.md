...
> 点击 **使用配置** 可以 [编译](#编译配置) 并启用该配置文件。该功能会对该配置使用的远程 [规则集](#规则集url)、[脚本](#脚本url) 等远程资源进行更新。软件版本大于等于 2.2.26(2621) 时，该按钮仅在非当前配置文件上存在

### [编译配置](#使用目录)

> 点击 **编译配置** 可以对该配置文件进行重新编译。可以理解为将该配置文件依据内置逻辑或解析器处理一遍，生成符合软件当前版本底层规范的内部使用格式。该功能会更新该配置文件所用到的 [规则集](#规则集url)、[脚本](#脚本url) 等远程资源。仅当软件版本大于等于 2.2.26(2621) 时才具有该功能
