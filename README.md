...
>
> 💡 **compatibility-mode**：网络兼容模式。`0 > 禁用`；`1 > Proxy with Loopback Address`；`2 > Proxy Only`；`3 > TUN Only`；`5 > No Default Route`。当参数的值设定为3时的效果等同于启用 [Tun模式](#代理类型)：[设置](#设置页面) > 代理 > [代理类型](#代理类型) > None。当参数的值设定为5时的效果等同于启用 [兼容模式](#兼容模式)：[设置](#设置页面) > 代理 > [兼容模式](#兼容模式)
>
