...
### [隐藏参数](#使用目录)
> 
> **以下参数为隐藏属性，建议谨慎设置。相关选项仅支持在配置文件纯文本编辑模式中的 `[General]` 字段进行设置，不提供 UI 操作选项**
> 
...
> 自版本 `2.2.81 (3096)` 起支持通过 HTTP/2 协议进行中间人攻击来解密 HTTPS 流量。在配置文件或模块纯文本模式中的 `[MITM]` 字段中使用 `h2 = true` 表示启用
> 
> 打开HTTPS解密方法：
> 
