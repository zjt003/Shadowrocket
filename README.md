...
> 
> **若多终端使用同一个配置文件，需要使用证书 `ⓘ` 图标的复制粘贴功能，将同一个证书在多个终端上进行安装信任，**
...
> 
> Shadowrocket 的 **编辑参数 / 传递参数** 功能，是用于对请求或响应中的参数进行动态修改的一个高级功能，可用于配合模块中的 `URL Rewrite`、`Script`、`MITM` 等代码使用，目的是实现对网络请求的精准控制与定制化处理。编辑参数所修改或填写的内容不会因为模块更新而丢失
>
> 示例：
> 
> ```ruby
