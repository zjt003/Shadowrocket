...
> > ```
> > > `其中所用的 UUID 值需要在中转节点 JSON 文本中复制。该设置方式后期可能有变动`
> > 
...
> * 该功能支持使用整个订阅作为中转代理链，其生效节点是在该订阅的节点中随机选择。具体生效节点可在 VPN 日志搜索 `backend chain` 确认
> * 该功能支持批量修改订阅节点，实现同时部署多个节点的代理链。具体命令可查看 [订阅节点筛选](#订阅节点筛选) 中的相关指令
