...
> 若使用时需要切换或更换配置文件，且原配置已开启过 [https解密](#https解密)，则需要对新的配置文件重新安装并信任证书，才可使解密继续生效。为免除重复安装 CA 证书，可以选择使用证书模块。证书模块的意义是：复制一份已安装的证书到模块内，使其独立于配置文件，不受以后变更配置文件的影响，其原理是“模块的写法与配置文件相同，且模块的优先级高于配置文件”
> 
