...
> > * 当规则匹配后，如果策略是一个代理节点，流量将通过指定的代理节点进行连接。如果策略是一个代理分组，流量将根据代理分组的类型和设置，选择合适的代理服务器。如果策略是 **拒绝** 类型，则流量将被拒绝连接
> 
> **代理分组的子策略**
> > * **代理分组** 中可以包含 **代理节点** 或 **其他代理分组**，形成包含子策略的层级结构
> >   * *例如：代理分组类型为 **Select 手动选择**，它包含了 **节点-01** 和 **节点-02** 两个节点，用户可以手动选择使用其中的哪个代理节点来设定该分组的策略*
> > * **代理分组** 中可以设置经过多层代理分组的嵌套，形成多层级结构，由末端代理分组最终确定具体的代理服务器节点
> >   * *例如：代理分组类型为 **Select 手动选择**，它包含了 **美国地区** 和 **日本地区** 两个分组，用户可以手动选择使用其中的哪个代理分组来设定该分组的策略*
> > * **需要注意**，嵌套的代理分组的子策略中，仅推荐在 **末端代理分组** 中使用 **除 Select 之外的其他代理分组类型**，以免产生不必要的自动切换
