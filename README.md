...
> 
> 配置行设置语法与 [通用参数](#通用参数) 相同，格式为 `key = value`
...
> Shadowrocket 支持本地自定义 DNS 映射。包含 **本地值** 和 **DNS服务器** 两种设定方式。在默认情况下，对于代理类的 DNS 解析始终在远端服务器上执行，支持使用 `use-local-host-item-for-proxy` 等 [隐式参数](#隐式参数)
>
...
> 修改 HTTP(S) 请求/响应，动态拦截并修改请求的 URL 或内容。重写类型包含 `HEADER`、`302`、`307` 以及其他支持的代理策略。支持使用 `always-reject-url-rewrite` 等 [隐式参数](#隐式参数)
