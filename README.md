...
> 该选项仅当配置文件是以远程配置文件的形式添加进来时才会出现。其作用是使用远程配置替换当前配置文件，同时可以更新当前所用的规则集、脚本等远程资源。该功能一般适用于在远端维护的配置文件，并使用远程配置对本地配置进行完整的更新替换，该操作会覆盖对本地配置文件所做的自定义设置，对于在本地维护或修改的配置文件并不适用此功能。软件同时提供自动更新功能，详细更新设置技巧参见 [自动更新](#自动更新) 词条
