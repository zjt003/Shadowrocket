...
> 现阶段软件的自动更新包含配置文件自动更新、服务器订阅自动更新以及GEOIP数据库自动更新。需要在“系统设置 - 通用 - 后台App刷新”中为Shadowrocket启用后台刷新
> 
> * **配置**：该功能可根据设置自动更新当前配置文件以及当前所用的规则集、脚本等远程资源。若当前配置文件使用的是从远程链接加载的，更新配置文件会使用原远程链接拉取全新配置替换当前配置，该操作会覆盖对该配置文件做过的所有自定义设置
> 
