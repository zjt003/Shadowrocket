...
> [!CAUTION]
> **更新配置会覆盖在本地对配置文件所做的自定义设置，对于在本地维护或修改的配置文件而言并不推荐使用此功能**

> [!TIP]
> **小技巧：若当前配置是远程配置，希望使用 [自动更新](#自动更新) 功能定期规则集、脚本集等远程资源且不希望自定义设置被覆盖或丢失时，可进入 [纯文本编辑](#编辑纯文本) 删除或注释掉 `update-url = *`，再使用 [自动更新](#自动更新) 功能。或者也可以考虑使用 [扩展配置/包含配置](#扩展配置)**
